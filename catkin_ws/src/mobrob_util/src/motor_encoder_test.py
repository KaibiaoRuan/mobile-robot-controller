...
def getEncoderRawValues(): #clears out the serial port and waits for the next encoder values
    ...
    while (not (e0_found and e1_found)):
        try:
            ...
            print("Unable to find AlaMode/Arduino. Please try again, check the serial port (name, function), or reflash Alamode/Arduino.")
            exit();
    return [e0_value,e1_value]

ENCODER_MOVE_THRESH = 1000;  #counts to move before encoder is considered 'moved'

ser = serial.Serial(serial_device_name)  #serial port to alamode
ser.baudrate = 115200
ser.bytesize = 8
ser.parity = 'N'
ser.stopbits = 1
